...
export default function CmsAdmin() {
  ...
  return <div className="cms-shell" dir="ltr" lang="en"><AdminNavigation /><main className="cms-main admin-page">
    ...
    {!loading && role && section && <>
      <p className="admin-permission">{role === 'viewer' ? 'Read only — Viewer access. Only administrators can save or delete.' : 'Administrator access. English and Arabic content are edited together.'}</p>
      ...
      <div className={`cms-workspace ${section.singleton ? 'cms-singleton' : ''}`}>
        ...
        </div>)}</div><div className="cms-actions"><button type="submit">{busy ? 'Saving…' : 'Save changes'}</button>{section.table === 'stores' && original && <button type="button" className="cms-delete" onClick={() => void remove()}>Delete store</button>}</div></fieldset></form> : <p className="cms-note">{role === 'viewer' ? 'Select a record to view its details.' : 'Select a record or add a new one.'}</p>}
      </div>
    </>}
  </main></div>
}
