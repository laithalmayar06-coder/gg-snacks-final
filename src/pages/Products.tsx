...
export default function Products() {
  ...
  return <ProductPageLayout><div ref={root} className="catalogue-content"><header className="catalogue-heading"><p className="catalogue-kicker">{t.catalogueLabel}</p><h1>{t.catalogueTitle}</h1><p className="catalogue-description">{t.catalogueDescription}</p></header><div className="catalogue-grid">{productFamilies.map((product, index) => <Link to={`/products/${product.slug}`} key={product.id} className="catalogue-world" style={{ '--catalogue-accent': product.accentColor } as CSSProperties} aria-label={`${t.catalogueExplore} ${product.name}`}><PackagingStage product={product} /><div className="catalogue-world-caption"><div><span className="catalogue-index" aria-hidden="true">{String(index + 1).padStart(2, '0')} / GG</span><h2 dir="ltr">{product.name}</h2><p>{product.shortDescription?.[language] || t.catalogueDescriptor}</p></div><span className="catalogue-explore">{t.catalogueExplore}<span aria-hidden="true">↗</span></span></div></Link>)}</div></div></ProductPageLayout>
}
